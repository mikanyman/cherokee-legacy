<?php
class KTFolderTransactionProxy extends KTFolderTransaction {
    ...
    function getbyfolder() { $aArgs = func_get_args(); return $this->_callOnObject("getbyfolder", $aArgs); }

    function getbyuser() { $aArgs = func_get_args(); return $this->_callOnObject("getbyuser", $aArgs); }

    function &_fetch() {
        if (!empty($GLOBALS["_OBJECTCACHE"]["KTFolderTransaction"][$this->iId])) {
            $oObject =& $GLOBALS["_OBJECTCACHE"]["KTFolderTransaction"][$this->iId];
            return $oObject;
        }
        $oObject =& new KTFolderTransaction;
        $res = $oObject->load($this->iId);
        if (PEAR::isError($res)) {
            return $res;
        }
        $GLOBALS["_OBJECTCACHE"]["KTFolderTransaction"][$this->iId] =& $oObject;
        return $oObject;
    }
    ...
    function KTFolderTransactionProxy ($iId) { $this->iId = $iId; }
