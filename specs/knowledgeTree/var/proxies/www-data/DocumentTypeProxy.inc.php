<?php
class DocumentTypeProxy extends DocumentType {
    ...
    function documenttype() { $aArgs = func_get_args(); return $this->_callOnObject("documenttype", $aArgs); }
    ...
    function isused() { $aArgs = func_get_args(); return $this->_callOnObject("isused", $aArgs); }

    function getfieldsets() { $aArgs = func_get_args(); return $this->_callOnObject("getfieldsets", $aArgs); }

    function getlist() { $aArgs = func_get_args(); return $this->_callOnObject("getlist", $aArgs); }

    function getlistforuserandfolder() { $aArgs = func_get_args(); return $this->_callOnObject("getlistforuserandfolder", $aArgs); }
    ...
    function &_fetch() {
        if (!empty($GLOBALS["_OBJECTCACHE"]["DocumentType"][$this->iId])) {
            $oObject =& $GLOBALS["_OBJECTCACHE"]["DocumentType"][$this->iId];
            return $oObject;
        }
        $oObject =& new DocumentType;
        $res = $oObject->load($this->iId);
        if (PEAR::isError($res)) {
            return $res;
        }
        $GLOBALS["_OBJECTCACHE"]["DocumentType"][$this->iId] =& $oObject;
        return $oObject;
    }
    ...
    function DocumentTypeProxy ($iId) { $this->iId = $iId; }
