<?php
class KTPermissionObjectProxy extends KTPermissionObject {
    ...
    function &_fetch() {
        if (!empty($GLOBALS["_OBJECTCACHE"]["KTPermissionObject"][$this->iId])) {
            $oObject =& $GLOBALS["_OBJECTCACHE"]["KTPermissionObject"][$this->iId];
            return $oObject;
        }
        $oObject =& new KTPermissionObject;
        $res = $oObject->load($this->iId);
        if (PEAR::isError($res)) {
            return $res;
        }
        $GLOBALS["_OBJECTCACHE"]["KTPermissionObject"][$this->iId] =& $oObject;
        return $oObject;
    }
    ...
    function KTPermissionObjectProxy ($iId) { $this->iId = $iId; }
